...
class StopWorkstationScriptTests(unittest.TestCase):
    @staticmethod
    def _args() -> object:
        """Build common argument payload used by builder stop tests."""
        return type(
            "Args",
            (),
            {
    ...
    def test_main_destroys_without_save_when_save_flag_is_unset(self) -> None:
        """Expected: wrapper delegates default builder stop flow without save-on-stop."""
        session = Mock()
    ...
    def test_main_passes_ami_save_inputs_when_enabled(self) -> None:
        """Edge: wrapper forwards builder AMI save options to shared orchestration."""
        session = Mock()
        session.region_name = "us-west-2"
        session.client.side_effect = [Mock(), Mock()]
        with (
        ...
        self.assertEqual("builder", inputs.environment_key)
